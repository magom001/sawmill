...
class TransferReportsTable extends Component {
  ...
  render() {
    if (this.props.data.loading) {
      ...
      if (docs.length === 0) {
        rows = <Table.Row>
          <Table.Cell colSpan={5} textAlign="center">Данных нет</Table.Cell>
        </Table.Row>
      } else {
        rows = docs.map((d, i) => {
          return (
        ...
        <Table celled selectable size="small">
          <Table.Header>
            <Table.Row>
              ...
              <Table.HeaderCell>склад оприходования</Table.HeaderCell>
            </Table.Row>
          </Table.Header>
          <Table.Body style={{
            cursor: 'pointer'
...
TransferReportsTable = withRouter(TransferReportsTable);
